    ...
    class WriteMessageViewController: UIViewController {
        ...
        let tabBarButton = UIBarButtonItem(image: UIImage(systemName: "arrow.backward.circle"), style: .plain, target: self, action: #selector(backButtonTapped))
    ...
    extension WriteMessageViewController {
        ...
        @objc func sendMessage() {
            guard messageTextView.text.count >= 21 else {
                showShortMessageAlert()
                return
            }
            ...
            APIManager.shared.sendMessage(content: messageTextView.text, accessToken: accessToken) { [weak self] result in
                    ...
                    case .success(let isSuccess):
                        self?.handleSendMessageSuccess(isSuccess)
                    case .failure(let error):
                        self?.handleSendMessageFailure(error)
                }
            }
        }
    ...
    extension WriteMessageViewController {
        private func showShortMessageAlert() {
            let alertController = UIAlertController(title: "짧은 편지", message: "편지 내용을 20자 이상으로 작성해주세요 :)", preferredStyle: .alert)
            let okAction = UIAlertAction(title: "OK", style: .default, handler: nil)
            alertController.addAction(okAction)
            present(alertController, animated: true, completion: nil)
        }

        private func handleSendMessageSuccess(_ isSuccess: Bool) {
            if isSuccess {
                print("Message sent successfully!")
                showSendCompletionView()
            } else {
        ...
        private func showSendCompletionView() {
...
//                sendCompletionVC.modalTransitionStyle = .crossDissolve
//                sendCompletionVC.modalPresentationStyle = .fullScreen
